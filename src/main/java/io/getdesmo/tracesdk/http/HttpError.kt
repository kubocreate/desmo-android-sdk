 ...
 *
 * Mirrors the Swift HTTPError enum.
 */
sealed class HttpError : Exception() {
     ...
     *
     * We avoid naming the property `cause` because `Exception` already has
     * a `cause` member; reusing that name would hide the superclass member.
     */
