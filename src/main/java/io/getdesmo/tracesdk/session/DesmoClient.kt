...
class DesmoClient(
    ...
    private val telemetry: TelemetryProvider =
        if (context != null) {
            HttpTelemetryProvider(context, httpClient, config.loggingEnabled)
        } else {
    ...
    suspend fun startSession(
        ...
        if (config.loggingEnabled && anchorPosition != null) {
            Log.d(TAG, "Anchor position captured: ${anchorPosition.lat}, ${anchorPosition.lng}")
        }

        // Get sensor availability
        val sensorAvailability = telemetry.getSensorAvailability()
        if (config.loggingEnabled) {
            Log.d(TAG, "Sensor availability: accel=${sensorAvailability.accelerometer}, gyro=${sensorAvailability.gyroscope}, baro=${sensorAvailability.barometer}, gps=${sensorAvailability.gps}")
        }
